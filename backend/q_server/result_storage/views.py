...
class ResultsListView(APIView):
    def get(self, request):
        ...
        results_data = Result.objects.order_by( (1 +F('day_count')*F('day_count')/100)* F('sum') / F('day_count')).reverse()
...
class MixResultsListView(APIView):
    def get(self, request):
        ...
        results_data = MixResult.objects.order_by( (1 + F('day_count')*F('day_count')/100)* F('sum') / F('day_count')).reverse()
        results_data = results_data[ 0:100 ]
