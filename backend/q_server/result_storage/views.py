...
class ResultsListView(APIView):
    def get(self, request):
        ...
        results_data = Result.objects.order_by(F('percent') + Sqrt(F('sum')) /30/ F('day_count')).reverse()
...
class MixResultsListView(APIView):
    def get(self, request):
        ...
        results_data = MixResult.objects.order_by(F('percent') + Sqrt(F('sum')) /30/ F('day_count')).reverse()
        results_data = results_data[ 0:100 ]
